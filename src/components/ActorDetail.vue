...
<v-card :elevation="hover ? 16 : 2"
    ...
    >
      ...
      <h4 class="mt-1 grey--text">Conocido por</h4>
      <v-row>
           <v-col cols="12" sm="2" :key="movie.id" v-for="movie in this.knownFor" class="mt-5">
                 <v-hover
        v-slot="{ hover }"
        open-delay="200"
      >
<v-card :elevation="hover ? 16 : 2"
        ...
        <h2 class="mt-2 grey--text ">Creditos</h2>
        <ul class=" pl-5 mt-8">
  ...
  methods: {
    async fetchActor(actorId) {
      const response = await this.$http.get(
        "https://api.themoviedb.org/3/person/" + actorId + "?language=es-US"
      );
    ...
    async fetchCredits(actorId) {
      const response = await this.$http.get(
        "https://api.themoviedb.org/3/person/" + actorId + "/combined_credits" + "?language=es-US"
      );
